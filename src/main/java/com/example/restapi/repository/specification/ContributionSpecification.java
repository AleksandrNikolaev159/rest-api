...
import com.example.restapi.service.util.DateSearchUtils;
...
public class ContributionSpecification implements Specification<Contribution> {
    ...
    public Predicate toPredicate(Root<Contribution> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        ...
        if(contribution.getId()!=null){
            predicates.add(criteriaBuilder.equal(root.get("id"), contribution.getId()));
        }
        if(contribution.getClient() !=null){

            if(contribution.getClient().getId() != null){
                predicates.add(criteriaBuilder.like(criteriaBuilder.upper(root.get("client").get("id")), "%" + contribution.getClient().getId() + "%".toUpperCase()));
            }
        ...
        if(contribution.getBank() != null){
            if(contribution.getBank().getId() != null){
                predicates.add(criteriaBuilder.like(criteriaBuilder.upper(root.get("bank").get("id")), "%" + contribution.getBank().getId() + "%".toUpperCase()));
            }
            ...
            if(contribution.getBank().getBik() != null){
                predicates.add(criteriaBuilder.like(criteriaBuilder.upper(root.get("bank").get("bik")), "%" + contribution.getBank().getBik() + "%".toUpperCase()));
            }
        }
